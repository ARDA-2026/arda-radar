...
# 계수만 교체한다.
#
# sphere_drone(드론을 새로 가정해 낙하 물체와 함께 녹화, 드론 자체가 낙하로
# 오판정된 hard negative 2건 포함) 배치 추가 후 84건(진짜 49 / 노이즈 35)으로
# 재학습. 드론 궤적은 자동 판정에서 fell=True로 걸린 경우에만 라벨링했다 —
# _model_confidence()는 실제 배포 코드에서 확정 순간에만 호출되므로, 한 번도
# 확정되지 않은 궤적을 억지로 라벨링하면 라이브에서 절대 나오지 않는 특징
# 분포를 학습시키는 꼴이라(학습/서빙 시점 불일치의 또 다른 변형) 제외했다.
# LIKELY_REAL_THRESHOLD=0.24에서 재현율 1.00(FN=0) 유지하면서 정밀도가
# 0.79→0.83으로 개선돼, 임계값은 그대로 두고 계수만 교체한다.
